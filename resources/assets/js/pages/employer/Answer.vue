            ...
            <el-row>
                <el-col :span="18">
                    <div class="WorkLists" v-if="works.length > 0">
                        <WorkList v-for="work in works" :work="work" class="WorkList" action="answer"></WorkList>
                    </div>
                    <el-card class="EmptyState" v-else>
                        <div class="EmptyState-inner">
                            <i class="fa fa-comments-o EmptyState-icon"></i>
                            <span>您暂时还没有等待回复的咨询</span>
                        </div>
                    </el-card>
                </el-col>
                <el-col :span="6">
    ...
    export default {
        ...
        methods: {
            getReviewingWorks:function () {
                let that = this;
                this.$axios.get('/api/employer/works?status=1&has_questions=true').then( response => {
